...
const dataDir = mkdtempSync(join(tmpdir(), 'dsh-nu-shot-'))
...
try {
  ...
  await sleep(1500)
  // 打开新增账号弹窗
  await ev(`(() => { document.getElementById('openCreateBtn')?.click(); return 1 })()`)
  await sleep(700)
  const shot = await send('Page.captureScreenshot', { format: 'png' })
  if (shot?.result?.data) writeFileSync(join(outDir, 'modal-user-create.png'), Buffer.from(shot.result.data, 'base64'))
  const dim = await ev(`(() => { const el = document.querySelector('#nuModal .modal'); if (!el) return null; const r = el.getBoundingClientRect(); return { w: Math.round(r.width), h: Math.round(r.height) } })()`)
  console.log('新增账号弹窗:', JSON.stringify(dim))
  ws.close()
} finally {
  try { gw.kill() } catch { }
  try { edge.kill() } catch { }
